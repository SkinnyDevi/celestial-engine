...
void build_grid_mesh(RenderState *state) {
  ...
  Vec3 *vertices = calloc((size_t)vertexCount, sizeof(Vec3));
  int index = 0;

  for (int i = 0; i <= gridSize; ++i) {
    float t = ((float)i / (float)gridSize - 0.5f) * (halfExtent * 2.0f);
    vertices[index++] = (Vec3){-halfExtent, 0.0f, t};
    vertices[index++] = (Vec3){halfExtent, 0.0f, t};
    vertices[index++] = (Vec3){t, 0.0f, -halfExtent};
    vertices[index++] = (Vec3){t, 0.0f, halfExtent};
    vertices[index++] = (Vec3){-halfExtent, t, 0.0f};
    vertices[index++] = (Vec3){halfExtent, t, 0.0f};
  }
  ...
  id<MTLBuffer> vertexBuffer =
      [device newBufferWithBytes:vertices
                          length:vertexCount * sizeof(Vec3)
